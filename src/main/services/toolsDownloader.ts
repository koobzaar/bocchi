...
export class ToolsDownloader {
  ...
  async getLatestReleaseInfo(): Promise<{ downloadUrl: string; version: string }> {
    try {
      ...
      throw new Error(`Failed to get latest release info: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async downloadAndExtractTools(onProgress?: (progress: number) => void): Promise<void> {
    try {
      ...
      const sourceExists = await fs.promises.access(cslolToolsSource)
        .then(() => true)
        .catch(() => false)
      ...
      throw new Error(`Failed to download and extract tools: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
