...
export class SkinDownloader {
  ...
  async downloadSkin(url: string): Promise<SkinInfo> {
    ...
    try {
      ...
      throw new Error(`Failed to download skin: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private parseGitHubUrl(url: string): SkinInfo {
    ...
    const urlPattern = /github\.com\/darkseal-org\/lol-skins\/blob\/main\/skins\/([^\/]+)\/([^\/]+)$/;
    const match = url.match(urlPattern);
    
    if (!match) {
      throw new Error('Invalid GitHub URL format. Expected format: https://github.com/darkseal-org/lol-skins/blob/main/skins/[Champion]/[SkinName].zip');
    }
